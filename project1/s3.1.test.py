...
target=[0.2,0.0685,0.266,0.0173]
KP=[30,60,0,50]
KI=[1,1,0,0]
triglist=[0,1,0,0]
...
ad=ads.ADS1256()
#-----------------------------------------------------------------------------
#processes for rotation:
q0 = manager.Queue(1)
q1 = manager.Queue(1)
q2 = manager.Queue(1)
q3 = manager.Queue(1)
def Loop_for_rotation(index):
     if enable[index]==1:
          while True:
               if index == 0:
                    angle = q0.get()
               ...
               x[index].m.moverel(angle)
               print 'rotation done'
p0 = mp.Process(target= Loop_for_rotation, args = {0}).start()
#p1 = mp.Process(target= Loop_for_rotation, args = {1}).start()
#p2 = mp.Process(target= Loop_for_rotation, args = {2}).start()
#p3 = mp.Process(target= Loop_for_rotation, args = {3}).start()

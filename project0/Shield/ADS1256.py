...
class ADS1256:
    ...
    def SetEnableBuffer(self,enableflag):
	debug_print('setting buffer')
	self.chip_select()
	self.SendByte(self.CMD_WREG|0x00)
	self.SendByte(0x00)
	if enableflag==0:
		self.SendByte(0x00)
	else:
    ...
    def SetDRate(self,rate):
	debug_print('setting datarate')
	self.chip_select()
	self.SendByte(self.CMD_WREG|0x03)
	self.SendByte(0x00)
	if rate==0:
		self.SendByte(self.DRATE_30000)
	elif rate==1:
		self.SendByte(self.DRATE_15000)
	elif rate==2:
		self.SendByte(self.DRATE_7500)
	elif rate==3:
		print('---------------')
		self.SendByte(self.DRATE_2_5)
	else:
    ...
    def SelfCalibrate(self):
        ...
        self.SendByte(self.CMD_SELFCAL)
        ...
        #self.SendByte(self.CMD_SYSOCAL)
        ...
        #self.SendByte(self.CMD_SYSGCAL)
	...
	timebase=time.time()
        ...
        #print('ReadADC result: ' + str(result1) + ' ' + str(result2) + ' ' + str(result3))
        ...
        if total <= 8388607:
            ...
            return total-8388607*2
    
    def ReadADCc(self,timelimit):
	datalist=[]
	self.chip_select()
	self.WaitDRDY()
	timebase=time.time()
	self.SendByte(self.CMD_RDATAC)
	self.DataDelay()
	while(time.time()-timebase<timelimit):
		...
		total=(result1<<16)+(result2<<8)+result3
		total=float(total)/float(10000)		
	
		datalist.append(total)
	self.SendByte(self.CMD_SDATAC)
	self.chip_release()
        lend=len(datalist)
	if lend>=2:
		datalist=datalist[:lend-1]
		lend=lend-1
	else:
		print('Cant finish measurement within timelimt')
		return -1
	ave=float(sum(datalist))/float(lend)
	print(datalist)
	return ave
